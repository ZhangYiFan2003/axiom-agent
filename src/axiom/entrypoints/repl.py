...
from axiom.agent import Agent, AgentOrchestrator, PlanExecuteAgent
...
async def start_repl(cwd: str, config: AxiomConfig) -> None:
    console = Console()
    ...
    history_path = Path.home() / ".axiom" / "history" / "prompt_history.txt"
...
async def _handle_slash(
    ...
    if command == "/help":
        ...
        console.print("Use `axiom mcp serve --transport stdio|http --port 3000` to expose tools.")
    elif command == "/task":
...
def _model_command(arg: str, console: Console, config: AxiomConfig) -> None:
    if not arg:
...
def _task_command(arg: str, console: Console) -> None:
    manager = DurableTaskManager(Path.home() / ".axiom" / "tasks" / "tasks.db")
    sub, _, rest = arg.partition(" ")
    if sub == "add" and rest:
...
def _approval_prompt(request: dict[str, Any], console: Console, config: AxiomConfig) -> str:
