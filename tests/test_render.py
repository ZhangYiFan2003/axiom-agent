...
from axiom.entrypoints.repl import _bottom_toolbar, _prompt_message
from axiom.render import RichRenderer


def test_banner_renders_axiom_home_layout():
    stream = StringIO()
    ...
    assert "A    X   X  III  OOO  M   M" in output
    assert "AAAAA    X     I  O   O M M M" in output
    assert "Axiom Agent Runtime v0.1.0" in output
...
def test_text_deltas_render_as_markdown_on_turn_complete():
    ...
    renderer.handle({"type": "text_delta", "text": "你好，我是 **Axiom Agent "})
    renderer.handle({"type": "text_delta", "text": "Runtime**\n\n- `read_file`\n- **网页搜索**"})
    ...
    assert "Axiom Agent Runtime" in output
    assert "read_file" in output
    assert "网页搜索" in output
    assert "Run Summary" not in output
    assert "**Axiom Agent Runtime**" not in output
