...
from axiom.skill import SkillContextBuffer, SkillRegistry, SkillStateStore
from axiom.tools.base import ToolContext
from axiom.tools.builtins import load_skill
...
def test_skill_registry_layers_and_state(tmp_path, monkeypatch):
    ...
    _write_skill(project / ".axiom" / "skills", "project-only", "project desc", "v2")
...
def test_load_skill_pushes_body_into_context_buffer(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    _write_skill(tmp_path / ".axiom" / "skills", "demo", "demo desc", "v1", body="demo body")
