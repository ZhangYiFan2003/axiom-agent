...
- Memory:
  ...
  - project memory files `AXIOM.md`, `.axiom/AXIOM.md`, local variants
- Skills:
  - built-in/user/project skill layers
  - user/project `.axiom/skills/*/SKILL.md`
  - `~/.axiom/skills.json` disabled-state store
...
- MCP:
  ...
  - `axiom mcp init-chrome`
  - `axiom mcp list`
  - Axiom Agent Runtime MCP server over stdio/http for built-in tools
- Chrome DevTools MCP:
...
uv run axiom mcp serve --transport http --port 3999
```
